...
function doSimple(template, content, values, valueStorage)
    ...
    if frontmatter and frontmatter.meta then
        ...
        if (url and description and img and title) then
            ...
            local rendered_template = liluat.render(compiled_template,
                { url = url, img = img, description = description, title = title })
    ...
    if frontmatter and (not frontmatter.draft or frontmatter.draft == false) then
        table.insert(gatheredSitemapData,
            {
                loc = content,
                lastmod = frontMatterDateToTimestamp(frontmatter.date, frontmatter),
